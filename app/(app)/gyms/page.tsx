  ...
  return (
    ...
    >
      <div className="relative flex h-[75px] items-center gap-2.5 bg-surface-2 px-3">
        ...
        <IconShield size={28} className="text-primary" />
        <div>
...
export default function GymsPage() {
  ...
  const mainGyms = gyms.filter((g) => g.kind === "main");
  const houseGyms = gyms.filter((g) => g.kind === "house");
    ...
    <div className="mx-auto w-full max-w-screen-sm">
      ...
      <div className="px-3 pt-3">
        <div className="flex items-center gap-2 rounded-full border border-border bg-surface-2 px-3 py-2 text-muted">
          <IconSearch size={15} />
          <span className="text-[13px]">Search gyms...</span>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-2.5 px-3 pb-4">
        {showMain && mainGyms.map((g) => <MainCard key={g.slug} gym={g} />)}

        {filter === "all" && houseGyms.length > 0 && (
        ...
        {showHouse && houseGyms.map((g) => <HouseCard key={g.slug} gym={g} />)}
      </div>
    </div>
  );
}
