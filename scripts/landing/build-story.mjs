...
(function () {
  ...
  function playOars(f, delay) {
    ...
    setTimeout(function () {
      ...
      var K = 0.45;   // peek size

      /* The peek pose is MEASURED, not predicted: our scale composes with the
         wheel transform the app has already put on each oar, so where an oar
         ends up at scale K cannot be worked out from its rect alone. Instead:
         transitions off, stand each oar at translate 0 / scale K, read where
         that actually puts it, and derive the exact translate that brings the
         blade tip level and the pack in close. All in one synchronous pass —
         nothing is painted in between. Translate is the outermost part of the
         composition, so it shifts the result one-to-one. */
      var saved = oars.map(function (o) {
        var s = { t: o.style.translate, k: o.style.scale };
        o.style.transition = "none";
        o.style.translate = "0px 0px";
        o.style.scale = String(K);
        return s;
      });
      var targets = oars.map(function (o) {
        var r = o.getBoundingClientRect();   // where scale K really stands
        var cx = r.left + r.width / 2;
        /* Pulled to under a fifth of its scaled spread — the pack sits within
           the phone's width — with blade tips ~90px clear of it. The piece
           adds its own depth per blade on top of this, which leaves the
           centre blade standing slightly proudest: the drawing's silhouette,
           kept rather than corrected. */
        return ((pcx - cx) * 0.82) + "px " + ((pr.top - 90) - r.top) + "px";
      });
      oars.forEach(function (o, i) {        // back to parked, still unpainted
        o.style.translate = saved[i].t;
        o.style.scale = saved[i].k;
      });
      void d.body.offsetWidth;
      /* The fan has a faint FAR side — three background blades the piece
         keeps at a fraction of full opacity. The drawing's peek is the five
         bright ones; the faint ones stay tucked away until the spread, and
         simply fade into their background places then. */
      oars.forEach(function (o, i) {        // transitions on: rise to the pose
        o.style.transition = "";
        var dimEl = o.querySelector("[data-dim]");   // a shade: 0 = bright,
        // ~.11 = the two outer arc blades (still part of the fan), ~.22 = the
        // three background blades that wait for the spread
        if (dimEl && parseFloat(d.defaultView.getComputedStyle(dimEl).opacity) > 0.15) return;
        o.classList.remove("pre");
        o.style.translate = targets[i];
        o.style.scale = String(K);
      });
    }, delay || 0);
    ...
    // The faint background blades surface here too, in their own places.
    setTimeout(function () {
      ...
      // Hand the oars back: with our class gone, nothing of ours is in the
      // way when the wheel starts turning again.
      setTimeout(function () {
      ...
      try {
        ...
        while (q.length && seen < 20000) {
          ...
          if (sn && sn.logic && ("rot" in sn.logic) && sn.logic.state &&
              typeof sn.logic.state.pinned === "boolean") {
            sn.logic.setState({ pinned: false });
            return;
          }
  ...
  function runFlightBack(sec, f, done) {
    ...
    // …and the oars tuck back in behind the phone before it lifts off
    parkOars(sec, d, shell);
