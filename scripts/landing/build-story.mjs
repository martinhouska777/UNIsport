...
(function () {
  ...
  function playOars(f, delay) {
    ...
    setTimeout(function () {
      ...
      var K = 0.4;    // miniature size
      var F = 0.3;    // how tightly the circle is drawn in (the piece's own
                      // depth transform widens whatever we ask for)
      ...
      /* The peek pose is MEASURED, not predicted: our scale composes with the
         ...
         that actually puts it, and derive the targets. All in one synchronous
         pass — nothing is painted in between. Translate is the outermost part
         of the composition, so it shifts the result one-to-one. */
      ...
      /* The peek is the WHOLE wheel in miniature: every position compressed
         toward the wheel's own centre — the same circle, closer together —
         then stood behind the phone, Harvard's blade at the middle peeking
         ~90px above it. The lower half hides behind the phone, which is the
         point: the formation is already there before it opens. */
      ...
      /* One correction round: stand each oar at its computed target and read
         where it ACTUALLY ends up — the piece styles its blades by depth, and
         that pushes whatever we ask for outward. Subtract the error. Still
         unpainted. */
