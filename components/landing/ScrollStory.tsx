  ...
  */
  ...
  return (
    <div ref={root} className="ls-story" id={id} data-story={id} data-phone-screens style={accentVar}>
      <div ref={stage} className="ls-stage">
        ...
        <div className="ls-copy">
          {beats.map((b, i) => (
            ...
            >
              ...
              <h2 className="font-display text-[clamp(28px,2.9vw,37px)] font-normal leading-[1.08] tracking-[-0.01em] text-balance text-l-text max-lg:text-[clamp(22px,5.8vw,27px)]">
                {b.head}
        ...
        <div ref={phoneCol} className="ls-phone-col">
          <div ref={wrap} className="ls-phone-wrap">
            <Phone
              ref={phoneEl}
              /* On a laptop 320px, down from 360 (owner, 2026-09-04). The
                 stories' phone had been the biggest on the page; it is a step
                 in a walkthrough, and the closer's — the one with the school's
                 letter beside it — is the one that should hold the eye.
              ...
              className="ls-phone w-[min(320px,44svh,86vw)] cursor-pointer max-lg:w-[min(84vw,53cqh)]"
              data-story-phone={id}
              role="button"
              tabIndex={0}
      ...
      {/* The scroll markers, one per beat, under the sticky stage. Each is
          also the beat's ADDRESS: the title card's list of steps links straight
          at them (#story1-b3). Landing on a marker's top leaves the middle of
          the screen inside it — every marker is at least a screen tall — so
          the beat it owns is the one that lights up. */}
