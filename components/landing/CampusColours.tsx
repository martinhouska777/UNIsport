...
import { schools, rgba, lift, SCHOOL_CYCLE_MS, readable } from "@/lib/landingSchools";

/*
  CAMPUS COLOURS â the closer of the student story.

  Ported natively from the "UNIsport Campus Colours" design piece (a bundled
  app that was iframed): the same layout â words, then phone beside a giant
  serif letter with the school's name under it, then eight dots â with real
  ...
  that school's own gyms â the Harvard capture, recolour-shifted by
  scripts/landing/recolor-shots.mjs and its three cards rewritten by
  patch-gyms.mjs (Payne Whitney for Yale, Dillon for Princeton, â¦). The Gyms
  ...
  (the first step sooner â see FIRST_MS),
  a click on a dot pins that school, and it un-pins once the section has
  scrolled out of view. It opens on Harvard â the colour the story's phone
  was wearing â and resets to Harvard when it leaves.
  ...
  â like a dot click â pins it, so the cycle stops arguing with the finger.
  ...
  that has played. Reduced motion: no cycling, no fades â a still Harvard,
  and the dots still work.
*/

// The page's one pace (lib/landingSchools.ts) â but the FIRST step (Harvard â
...
export default function CampusColours({
  ...
  managed?: boolean;
  /** A little taller than one screen, sticky inside â the flight lands here. */
  pinned?: boolean;
  /** Something to stand beside the piece â the feature rows. */
  aside?: ReactNode;
  ...
  // The way OUT is quicker than the way in â see .lc-out in globals.css.
  ...
  /* In view â cycle; out of view â stop, un-pin, and go back to Harvard so
  ...
  /* A swipe walks to the next/previous school and pins it, exactly as a dot
     click does â so a finger can outrun the 2.6s cycle. */
  const walk = (dir: 1 | -1) => {
    setHeld(true);
  ...
  useImperativeHandle(ref, () => ({
    ...
    arriveAfterFlight: () => {
      ...
      later(() => setCycling(true), 600); // â¦and only then does it start rotating
    },
    arriveInPlace: () => {
  ...
  // in lib/landingSchools.ts (owner, 2026-09-15 — the navies and Brown read
  // black, the orange and the light blue vanished). Harvard stays its own crimson.
      ...
      >
        ...
        <div className={`lc-words mb-6 max-w-[760px] text-center ${wordsPre ? "lc-pre" : ""}`}>
          ...
          {/* The sub ends on the one thing you can press in the student story:
              ...
              rest of the piece â content colour from lib/landingSchools, rule
              1's exception.

              TWO EVEN LINES (owner, 2026-09-04: "this text 2 lines"). It was
              already wrapping, but at 52ch the second line was the orphan
              "yours next." â 46ch plus text-balance instead of text-pretty
        ...
        {/* ââ Phone beside the letter (letter above the phone below 1024px) ââ */}
        {/* On a phone the whole piece â words, letter, phone, dots â fits one screen:
            ...
            was simply cut off â a measured 139px gone at 1440x620 (owner,
            ...
            as big as possibleâ¦ i dont want from a big phone in scrolling
            ...
            to pay for it â the words' lg:mb-11 is now just mb-6, the stick's
            lg:py-10 is lg:py-8 â which buys ~28px of height, and the cap is
            ...
            window, so R must clear (278 + 0.088*H) / 1.088 â 325 at 860, 328
            at 900. Checked with no overflow at 1900x860, 1536x864, 1440x900,
            1280x800, 1024x768 and 390x844. (1440x620 still overflows â by
        ...
        <div className="flex w-full flex-col items-center justify-center gap-5 lg:flex-row lg:gap-[60px]">
          ...
          >
            {/* No school-coloured glow behind the letter any more (owner,
                2026-09-13: "I don't want â¦ the schools to glow"). */}
            <div className="relative grid place-items-center">
              {animate && prev != null && (
                <span
                  ...
                  style={{ color: readable(schools[prev].ink) }}
                >
                  {schools[prev].letter}
                </span>
              )}
