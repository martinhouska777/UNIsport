...
import { COX_COLOR, defaultBoatName, type Boat } from "@/lib/varsity/coachLineup";
...
const fromLineup = (b: Boat): RaceBoat => {
  ...
  return { name: named || (coxId ? surname(coxId) : stroke ? surname(stroke) : b.badge), badge: b.badge, seats, coxId };
};

const blankPiece = (boats: number) => ({ times: Array<number | null>(boats).fill(null), swaps: [] as SwapPair[] });
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    ...
                    <span className="mt-0.5 block truncate text-[11px] text-muted">
                      {[dayLabel(r.date), r.period, badge, r.piece].filter(Boolean).join(" · ")}
                    </span>
                  </span>
                  <span className="flex-shrink-0 text-[13px] tabular-nums text-muted">
                    {res.by == null ? "—" : res.winner ? `+${secs(res.by)}` : "Level"}
                  </span>
                </button>
              );
            })}
...
function SeatRaceEditor({
  ...
  const [texts, setTexts] = useState<string[][]>(() =>
    initial.pieces.map((p) => p.times.map((t) => (t == null ? "" : formatTime(t)))),
  );
  ...
  const [lineups, setLineups] = useState<Partial<Record<"AM" | "PM", Boat[]>> | null>(null);

  useEffect(() => {
    let active = true;
    const d = new Date(`${r.date}T12:00:00`);
    Promise.all([fetchLineup(sessionKey(d, "AM")), fetchLineup(sessionKey(d, "PM"))]).then(([am, pm]) => {
      if (!active) return;
  ...
  const removeLastPiece = () => {
    ...
    if (switching != null && switching >= r.pieces.length - 1) setSwitching(null);
  };

  const setTime = (k: number, b: number, s: string) => {
    setTexts(texts.map((row, i) => (i === k ? row.map((v, j) => (j === b ? s : v)) : row)));
    setR({
      ...r,
      pieces: r.pieces.map((p, i) => (i === k ? { ...p, times: p.times.map((v, j) => (j === b ? parseTime(s) : v)) } : p)),
    });
  };

  const setSwaps = (k: number, swaps: SwapPair[]) =>
    setR({ ...r, pieces: r.pieces.map((p, i) => (i === k ? { ...p, swaps } : p)) });
    ...
    <Sheet full title={isNew ? "New seat race" : "Seat race"} onClose={onClose}>
      ...
      <div className="mt-3 flex flex-wrap items-center gap-1.5">
          ...
          return (
            <button
              ...
              className={`h-9 rounded-full border px-4 text-[13px] font-semibold ${
                on ? "border-text bg-text text-background" : "border-border bg-surface text-text"
              }`}
            >
              {p} lineup
            </button>
        ...
        {lineups && lineupChips.length === 0 && (
          <span className="px-1 text-[13px] text-muted">No lineup on this day</span>
        )}
      </div>
      ...
      {r.pieces.map((pc, k) => {
        ...
        const pairs = [...(r.pieces[k - 1]?.swaps ?? []), ...pc.swaps]
          .map(([a, b]) => [boatAt(r, k, a), boatAt(r, k, b)].sort((x, y) => x - y) as [number, number])
          .filter(([i, j], n, all) => i >= 0 && j >= 0 && all.findIndex(([x, y]) => x === i && y === j) === n);
        return (
          <div key={k} className="mt-6">
            <div className="mb-2 flex items-center justify-between px-0.5">
              <span className="text-[11px] font-semibold uppercase tracking-[0.12em] text-muted">Piece {k + 1}</span>
                ...
                <div key={bi} className="min-w-0">
                        ...
                        >
                          ...
                          <span
                            className={`min-w-0 flex-1 truncate text-[13px] ${
                              id && red.has(id) ? "font-semibold" : "font-medium"
                            } ${id ? "" : "text-muted"}`}
                          >
                            {id ? surname(id) : "—"}
                          </span>
                  ...
                  <input
                    ...
                    className={`mt-1.5 w-full rounded-xl border bg-surface px-3 py-2 text-center font-mono text-base text-text placeholder:text-muted ${
                      texts[k]?.[bi] && pc.times[bi] == null ? "border-danger" : "border-border"
                    }`}
                  />
                </div>
              ))}
            ...
            {pairs.map(([i, j]) => {
              ...
              return (
                <div key={`${i}-${j}`} className="mt-1.5 px-0.5 text-[13px] text-text">
                  {m === 0 ? `${r.boats[i].name} = ${r.boats[j].name}` : `${r.boats[w].name} > ${r.boats[l].name} · ${secs(Math.abs(m))}`}
                </div>
              );
            })}
            ...
            <div className="mt-2 flex flex-wrap items-center gap-1.5">
              {pc.swaps.map((s, n) => (
                ...
                >
                  ...
                  <button
                    type="button"
                    onClick={() => setSwaps(k, pc.swaps.filter((_, i) => i !== n))}
              ...
              <button
                ...
                className={`flex h-8 items-center gap-1.5 rounded-full border px-3 text-[12px] font-semibold ${
                  switching === k ? "border-text bg-text text-background" : "border-border bg-surface text-text"
                }`}
              >
                {switching === k ? (
                  "Done"
      ...
      {results.some((x) => x.by != null) && (
        <div className="mt-6">
          ...
          <div className="flex flex-col gap-1.5">
            {results.map((res, n) =>
              res.by == null ? null : (
                <div key={n} className="flex items-center gap-3 rounded-xl border border-border bg-surface px-3.5 py-3">
                  <span className="min-w-0 flex-1 truncate text-[14px] text-text">
                    {res.winner ? (
                      <>
                        <span className="font-semibold">{nameOf(res.winner)}</span>
      ...
      <div className="mt-6 flex flex-col gap-2">
        <Button full onClick={() => (r.boats.length ? onSave(r) : setNoLineup(true))}>
          Save
        </Button>
        {noLineup && <p className="text-center text-[13px] text-danger">Pick a lineup</p>}
        {!isNew &&
          (confirmDelete ? (
